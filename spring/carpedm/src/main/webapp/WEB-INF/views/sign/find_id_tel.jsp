...
<!-- <link href="/carpedm/resoures/css/layout.css" rel="stylesheet"> -->
<style>
/* 헤더 가운데 정렬 */
.tel_find {
	width: 80%;
...
.search_text #text_tel, .search_text #text_name {
	height: 25px;
	width: 95%;
	font-size: 15px;
	font-family: "Wanted Sans Variable";
    ...
    window.onload = function () {
        ...
        button.addEventListener('click', function () {
            ...
            if (username === "") {
                ...
                fetch('/carpedm/find_id_tel', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded'
                    },
                    body: `username=${"${encodeURIComponent(username)}"}&usertel=${"${encodeURIComponent(usertel)}"}`
                })
                ...
                .then(data => {
                    if (!data || data.message === "fail") {
                        ...
                        alert("해당 아이디는 " + data.message + "입니다.");
                    }
                });
            }
        });

        let textbox = document.getElementById("text_name");
        // Enter 키 이벤트 리스너 추가
        ...
        let textbox2 = document.getElementById("text_tel");
        // Enter 키 이벤트 리스너 추가
			...
			<div class="search">
				...
				<table class="search_text" cellpadding="5" cellspacing="1">
					<tr>
						...
						<td width="*"><input type="text" id="text_name"
							placeholder="이름을 입력해주세요." autofocus></td>
					</tr>
					<tr>
						<td class="sub">전화번호</td>
						<td><input type="text" id="text_tel"
