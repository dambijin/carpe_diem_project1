...
<script>
				...
				.addEventListener(
						...
						function(event) {
							...
							if (!event.target.checked) {
								...
								let allCount = document
										.querySelectorAll(".checkbox").length;
								let checkedCount = document
										.querySelectorAll(".checkbox:checked").length;
	...
	function sortTable(n, isNumeric) {
		...
		while (switching) {
			...
			for (i = 1; i < (rows.length - 1); i++) {
				...
				var xContent = isNumeric ? Number(x.textContent.trim())
						: x.textContent.trim();
				var yContent = isNumeric ? Number(y.textContent.trim())
						: y.textContent.trim();
	...
	function redirectPage() {
		...
		window.location.href = '/carpedm/mypage_bookcart?' + '&page=' + "1"
				+ '&perPage=' + perPage;
		...
		<div class="s_section2">
			<div class="left_section">
				<a href="/carpedm/mypage_loan_status">
					<button type="button" class="sub_but">대출 현황</button>
				</a><br> <a href="/carpedm/mypage_loan_history">
					<button type="button" class="sub_but">대출 내역</button>
				</a><br> <a href="/carpedm/mypage_reservation_list">
					<button type="button" class="sub_but">예약 목록</button>
				</a> <a href="/carpedm/mypage_wishbook_list">
					<button type="button" class="sub_but">
						희망도서<br>신청목록
					</button>
				</a> <a href="/carpedm/mypage_bookcart">
					<button type="button" class="sub_but">장바구니</button>
				</a>
			</div>
			<div class="right_section">
				...
				<div>
					...
					<table id="page1">
						<tr id="page1_tr">
							...
							<th style="cursor: pointer;" onclick="sortTable(4,true)">담은일자</th>
				...
				<div id="paging">
					<%-- 서블릿에서 불러온 페이징 정보 --%>
					<c:set var="total_count" value="${cart_count}" />
